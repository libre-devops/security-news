...
variable "grant_admin_consent" {
  description = "Grant the requested Graph application roles in this stack. Each grant IS tenant-wide admin consent, so the applier needs AppRoleAssignment.ReadWrite.All. Set false to request the roles without consenting, then use the grant_admin_consent_commands output."
  type        = bool
  default     = true
}
